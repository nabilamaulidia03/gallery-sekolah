@extends('layouts.studentGallery')
...
<section id="gallery" class="section-padding">
    <div class="container">
        <div class="welcome-banner mb-4">
            <h3 class="text-capitalize">Selamat Datang Kembali, {{ auth()->user()->name }}</h3>
            <h5 class="text-capitalize">di Galeri SMKN 4 Bogor</h5>
        </div>
    ...
    <div class="modal-content border-0 shadow-lg rounded-4" style="backdrop-filter: blur(10px); background: rgba(255,255,255,0.9);">
      ...
      <div class="modal-body">
        ...
        <form id="commentForm">
          ...
          <input type="hidden" name="name" value="{{ auth('student')->user()->name ?? 'Anonim' }}">
...
<div class="modal fade" id="shareModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-sm">
    <div class="modal-content border-0 shadow-lg rounded-4" style="backdrop-filter: blur(10px); background: rgba(255,255,255,0.9);">
      <div class="modal-header border-0 pb-0">
        <h5 class="modal-title fw-bold text-primary">
          <i class="bi bi-share me-1"></i> Bagikan Galeri
        </h5>
        <button type="button" class="btn-close shadow-none" data-bs-dismiss="modal"></button>
      </div>

      <div class="modal-body text-center">
        <input type="hidden" id="shareImageUrl">

        <div class="d-grid gap-2">
          <button class="btn btn-outline-success rounded-pill fw-semibold share-whatsapp">
            <i class="bi bi-whatsapp"></i> WhatsApp
          </button>
          <button class="btn btn-outline-primary rounded-pill fw-semibold share-facebook">
            <i class="bi bi-facebook"></i> Facebook
          </button>
          <button class="btn btn-outline-info rounded-pill fw-semibold share-twitter">
            <i class="bi bi-twitter"></i> Twitter
          </button>
          <button class="btn btn-outline-danger rounded-pill fw-semibold share-instagram">
            <i class="bi bi-instagram"></i> Instagram (Copy Link)
          </button>
          <button class="btn btn-outline-secondary rounded-pill fw-semibold share-copy">
...
$(document).ready(function() {
    ...
    $('.share-btn').on('click', function() {
        ...
        $('#shareImageUrl').val(imgUrl);
        $('#shareModal').modal('show');
    });

    // 🔗 Share ke WhatsApp
    $('.share-whatsapp').on('click', function() {
        let url = encodeURIComponent($('#shareImageUrl').val());
        let shareUrl = `https://api.whatsapp.com/send?text=Lihat foto ini: ${url}`;
    ...
    $('.share-twitter').on('click', function() {
        let url = encodeURIComponent($('#shareImageUrl').val());
        let shareUrl = `https://twitter.com/intent/tweet?url=${url}&text=Lihat foto keren ini!`;
        window.open(shareUrl, '_blank');
    });

    // 🔗 Share ke Instagram (hanya copy link, karena IG gak support share via URL)
    $('.share-instagram, .share-copy').on('click', function() {
        let url = $('#shareImageUrl').val();
        navigator.clipboard.writeText(url).then(() => {
            alert('Link galeri disalin ke clipboard! Kamu bisa paste di Instagram.');
        });
    });
